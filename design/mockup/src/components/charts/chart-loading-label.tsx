...
export function ChartLoadingLabel({
  ...
  return (
    ...
    >
      {/* bxpk: the registry's ShimmeringText is a looping gradient sweep; the
          page has no gradients, so the loading label is plain muted text. */}
      <span className="font-medium text-sm tracking-wide text-[var(--muted-foreground)]">
        {text}
      </span>
    </motion.div>
  );
